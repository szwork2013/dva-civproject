  ...
  */
  getChecked (list, capture) {
    ...
    if (this.state.status >= checked) {
      if (capture !== 3 || isEmpty(this.props.data.ChildNodes)) {
  ...
  renderMarks () {
    let className
    let { $deep, ChildNodes } = this.props.data
    let noChild = isEmpty(ChildNodes)
  ...
  render () {
    ...
    if (data.ChildNodes) {
      let items = data.ChildNodes.map(function (item, i) {
        return (
          <Item ref={i}
            key={item.$key}
            icons={icons}
      ...
      ChildNodes = <ul className={open && styles.open}>{items}</ul>
      icon = open ? icons[1] : icons[0]
      handle = (
        <div className={styles.expand_btn} onClick={this.toggle}></div>
      )
    } else {
      icon = icons[2]
    }
    ...
    return (
      <li>
        <div className={styles.tree_label}>
          {this.renderMarks()}
          {handle}
